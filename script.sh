...
wget "$ROOTFS_LINK" -O "$ROOTFS_TAR" &>/dev/null || exit 1
wget "$SYSTEM_LINK" -O "$AND_IMAGE" &>/dev/null || exit 1
...
rm target-system.img target-rootfs.img
mv "$IMAGE_DIR"/system.img target-system.img
mv "$IMAGE_DIR"/rootfs.img target-rootfs.img
